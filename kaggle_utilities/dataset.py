...
class GitHubCodeDataset(IterableDataset):
    ...
    When loop=True (default), loops through files indefinitely (infinite dataset).
    When loop=False, makes a single pass through the files (one epoch).
    """
    ...
    def _token_stream(self):
        """Yield tokens from files, optionally looping indefinitely."""
...
def build_data_loader(
    ...
    Args:
        file_paths: List of source file paths to train on.
        tokenizer_id: HuggingFace tokenizer to use.
        seq_len: Sequence length for training chunks.
        batch_size: Batch size for DataLoader.
        num_workers: DataLoader worker processes.
        loop: If True, dataset loops indefinitely. If False, single epoch.
